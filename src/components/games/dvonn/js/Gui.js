...
import { begin_number, end_number, begin_letter, end_letter, begin_diagonal_number, begin_diagonal_letter,
  end_diagonal_number, end_diagonal_letter, letters, Color, Coordinates, Phase, Move, State } from './Engine'

var Gui = function (c, e, l, g) {
// private attributes
  var _engine = e;
  var _color = c;
  var _gui = g;
...
// public methods
  ...
  this.draw = function () {
    ...
    if (_engine.phase() === Phase.MOVE_STACK && selected_piece && selected_piece.is_valid()) {
  ...
  this.get_move = function () {
    ...
    if (_engine.phase() === Phase.PUT_DVONN_PIECE) {
      move = new Move(Phase.PUT_DVONN_PIECE, _engine.current_color(), this.get_selected_coordinates());
    } else if (_engine.phase() === Phase.PUT_PIECE) {
      move = new Move(Phase.PUT_PIECE, _engine.current_color(), this.get_selected_coordinates());
    } else if (_engine.phase() === Phase.MOVE_STACK) {
      var e = _engine.clone();

      e.move_stack(this.get_selected_piece(), this.get_selected_coordinates());
      move = new Move(Phase.MOVE_STACK, _engine.current_color(), this.get_selected_piece(), this.get_selected_coordinates(), e.remove_isolated_stacks());
    }
    return move;
  };
...
// private methods
  ...
  var draw_coordinates = function () {
    ...
    for (var l = 'A'.charCodeAt(0); l < 'L'.charCodeAt(0); ++l) {
      pt = compute_coordinates(l, begin_number[l - 'A'.charCodeAt(0)]);
      pt[0] += 5;
      pt[1] -= 5;
    ...
    context.textBaseline = "bottom";
    for (var n = 1; n < 6; ++n) {
      pt = compute_coordinates(begin_letter[n - 1].charCodeAt(0), n);
      pt[0] -= 15 + (n > 3 ? 5 : 0);
      pt[1] -= 3;
  ...
  var draw_grid = function () {
    ...
    for (var l = 'A'.charCodeAt(0); l < 'L'.charCodeAt(0); ++l) {
      ...
      _pt_begin = compute_coordinates(l, begin_number[index]);
      _pt_end = compute_coordinates(l, end_number[index]);
      context.moveTo(_pt_begin[0], _pt_begin[1]);
      context.lineTo(_pt_end[0], _pt_end[1]);
    }

    for (var n = 1; n < 6; ++n) {
      _pt_begin = compute_coordinates(begin_letter[n - 1].charCodeAt(0), n);
      _pt_end = compute_coordinates(end_letter[n - 1].charCodeAt(0), n);
      context.moveTo(_pt_begin[0], _pt_begin[1]);
      context.lineTo(_pt_end[0], _pt_end[1]);
    }

    for (var i = 0; i < 11; ++i) {
      _pt_begin = compute_coordinates(begin_diagonal_letter[i].charCodeAt(0),
        begin_diagonal_number[i]);
      _pt_end = compute_coordinates(end_diagonal_letter[i].charCodeAt(0),
  ...
  var draw_state = function () {
    for (var index in _engine.get_intersections()) {
      ...
      if (intersection.state() === State.NO_VACANT) {
        ...
        if (n > 1) {
          ...
          if (intersection.color() === Color.WHITE) {
            context.fillStyle = "#000000";
          } else {
            context.fillStyle = "#ffffff";
          }
    ...
    if (_engine.current_color() === _color || _gui) {
      ...
      if (letter !== 'X') {
        ...
        if (number !== -1) {
          ...
          if (_engine.phase() === Phase.PUT_DVONN_PIECE && _engine.get_intersection(letter, number).state() === State.VACANT) {
            selected_coordinates = new Coordinates(letter, number);
            ok = true;
          } else if (_engine.phase() === Phase.PUT_PIECE && _engine.get_intersection(letter, number).state() === State.VACANT) {
            selected_coordinates = new Coordinates(letter, number);
            ok = true;
          } else if (_engine.phase() === Phase.MOVE_STACK && _engine.get_intersection(letter, number).state() === State.NO_VACANT) {
            if (selected_piece && selected_piece.is_valid()) {
              if (_engine.verify_moving(selected_piece, new Coordinates(letter, number))) {
                selected_coordinates = new Coordinates(letter, number);
                ok = true;
              }
            } else {
              ...
              if (_engine.can_move(coordinates)) {
                ...
                if (color !== Color.RED && _engine.current_color() === color) {
