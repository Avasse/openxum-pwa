...
import { Status } from './Manager'

var GamePage = function (namespace, n, fc, c, oc, gt, gi, m, u, oi, opi, r) {
// private attributes
  var canvas;
  var canvas_div;
  var engine;
...
// private methods
  ...
  var build_opponent = function (namespace, color, game_type, game_id, opponent_color, username, owner_id, opponent_id) {
    if (game_type === 'remote_ai') {
      ...
      if (engine.get_possible_move_list) {
        opponent = new MCTSPlayer(opponent_color, engine);
      } else {
  ...
  var build_manager = function (namespace) {
    manager = new namespace.Manager(engine, gui, opponent, new Status(document.getElementById("status")));
  };
